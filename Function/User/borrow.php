    ...
    if ($resultUser && mysqli_num_rows($resultUser) > 0) {
        ...
        if ($resultBorrowedBooks && mysqli_num_rows($resultBorrowedBooks) > 0) {
            ...
            if ($numBorrowedBooks < $batasBuku) {
                ...
                $checkAvailabilityQuery = "SELECT status_buku, jumlah_tersedia FROM buku WHERE id_buku = '$bookId'";
                $resultAvailability = mysqli_query($conn, $checkAvailabilityQuery);

                if ($resultAvailability && mysqli_num_rows($resultAvailability) > 0) {
                    $rowAvailability = mysqli_fetch_assoc($resultAvailability);

                    if ($rowAvailability['status_buku'] === 'Available' && $rowAvailability['jumlah_tersedia'] > 0) {
                        // Update status buku menjadi 'Borrowed' jika jumlah_tersedia > 0
                        $status = 'Borrowed';

                        // Update jumlah_tersedia
                        $newQuantity = $rowAvailability['jumlah_tersedia'] - 1;
                        $updateQuantityQuery = "UPDATE buku SET jumlah_tersedia = '$newQuantity' WHERE id_buku = '$bookId'";
                        $resultUpdateQuantity = mysqli_query($conn, $updateQuantityQuery);
